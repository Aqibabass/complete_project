        ...
        return (
            <div className="fixed inset-0 z-50 bg-white min-h-screen overflow-y-auto">
                <div className="flex justify-between items-center p-6 border-b">
                    <h2 className="text-2xl md:text-3xl font-semibold">{trip?.userSelection?.location?.label}</h2>
                    <button
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 p-6">
                    {photo.map((pic, index) => (
                        <a key={index} href={pic} target="_blank" rel="noopener noreferrer" className="w-full h-64 md:h-80 lg:h-96">
                            <img
            ...
            <div className="my-5 flex flex-col gap-2">
                ...
                <div className="flex items-center justify-between">
                    <div className="flex flex-col md:flex-row gap-5">
                        ...
                        <h2 className="p-1 px-3 bg-gray-200 rounded-full text-gray-700 text-md md:text-md flex items-center gap-2">
                            <IoPeopleSharp /> No of Travellers {trip?.userSelection?.traveller}
                        </h2>
                    </div>
                    <div>
                        <Button className="bg-secondary hover:bg-secondary" onClick={() => setConfirmDelete(true)}>
                            Delete Trip
                        </Button>
                    </div>
                </div>
                ...
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
                    <div className="bg-white p-6 rounded-md shadow-md">
                        ...
                        <div className="flex justify-end mt-4">
                            <button className="mr-4 px-4 py-2 bg-gray-300 rounded" onClick={() => setConfirmDelete(false)}>
                                Cancel
                            </button>
                            <button className="px-4 py-2 bg-red-500 text-white rounded" onClick={deleteTrip}>
                                Delete
                            </button>
                        </div>
                    </div>
